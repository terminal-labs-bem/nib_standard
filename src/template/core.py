...
class AppContext(Immutable):
    ...
    def __init__(self):
        ...
        def get_invocation_cmd():
            full_invocation_cmd = sys.argv[0] + " " + ' '.join( [pipes.quote(s) for s in sys.argv[1:]])
            invocation_cmd = full_invocation_cmd.replace(os.environ['VIRTUAL_ENV'] + "/bin/", "")
        ...
        self.hard_path_to_python = str(os.path.abspath(os.path.realpath(sys.executable)))
        ...
        self.compiler_toolchain = str("build-essential (libc6-dev | libc-dev, gcc, g++, make, dpkg-dev)")
        self.compilor_for_c_extensions = str(distutils.sysconfig.get_config_vars()["CC"])
        self.platform = str(platform.platform())
        self.distro_prettyname = str(platform.freedesktop_os_release()["PRETTY_NAME"])
        self.distro_id_like = str(platform.freedesktop_os_release()["ID_LIKE"])
...
def initapp(appcontext):
    ...
    t.add_rows([['check', 'result'], 
        ...
        ['compilor for c extensions', appcontext.compilor_for_c_extensions],
        ['platform', appcontext.platform],
        ['distro pretty name', appcontext.distro_prettyname],
        ['distro id like', appcontext.distro_id_like],
    ])
    print(t.draw())
    ## PYTHONPYCACHEPREFIX
